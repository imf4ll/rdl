...
pub fn get_video(url: String) -> Vec<ValidFormat> {
    let video_id_vec: Vec<&str> = url.split("/").collect();
    let video_id = video_id_vec[5];

    let guest_token = get_token();
    ...
    let first: Vec<&str> = formats_res.split("\"variants\":").collect();
    let last: Vec<&str> = first[1].split("}}]},\"favorite_count\"").collect();

    let formats: Vec<Format> = serde_json::from_str(last[0]).unwrap();
    let mut qualities: Vec<ValidFormat> = vec![];
